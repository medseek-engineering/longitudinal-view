(function() {
  ...
  chart.settings.w =  chart.settings.w -
                      chart.settings.margin.left -
                      chart.settings.margin.right;

  chart.settings.h =  chart.settings.h -
                      chart.settings.margin.top -
                      chart.settings.margin.bottom;

  height = chart.settings.h +
           chart.settings.margin.top +
           chart.settings.margin.bottom;
  width =  chart.settings.w +
           chart.settings.margin.left +
           chart.settings.margin.right;
  ...
  dateRange = d3.extent(chart.dates);

  minN = dateRange[0].getTime();
  ...
  chart.xAxis = chart.svg
    ...
    .attr('transform',
          'translate(0, '+(chart.settings.h + 0) + ')');
  ...
  zoom = d3.behavior.zoom()
    ...
    .on('zoom', function() {
      ...
      if (chart.xScale.domain()[0] < minDate) {
        x = zoom.translate()[0] -
            chart.xScale(minDate) +
            chart.xScale.range()[0];
        zoom.translate([x, 0]);
      } else if (chart.xScale.domain()[1] > maxDate) {
        x = zoom.translate()[0] -
            chart.xScale(maxDate) +
            chart.xScale.range()[1];
        zoom.translate([x, 0]);
      }
      update();
    });
