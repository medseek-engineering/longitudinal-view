...
(function() {
  ...
  function redrawChart() {
    ...
    //chart.svg.select('.axis-x').call(chart.smallTicks);
    //chart.svg.select('.axis-x').call(chart.largeTicks);
  }
  ...
  var zoom = d3.behavior.zoom()
    ...
    .on('zoom', function() {
      ...
      var x = zoom.translate()[0] - chart.xScale(minDate) + chart.xScale.range()[0];
            zoom.translate([x, 0]);
        } else if (chart.xScale.domain()[1] > maxDate) {
      var x = zoom.translate()[0] - chart.xScale(maxDate) + chart.xScale.range()[1];
            zoom.translate([x, 0]);
        }
        redrawChart();
  ...
  var overlay = d3.svg.area()
    .x(function(d) { return chart.xScale(d); })
    .y0(0)
    .y1(chart.settings.h);
